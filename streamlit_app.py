...
# --- 3. NAVIGATION ---
page = st.sidebar.radio("Navigation", ["📊 Dashboard", "📜 Order Logs", "🎨 Design Vault", "📤 Supplier Export"])

# --- PAGE: ORDER LOGS (VIEWER) ---
if page == "📜 Order Logs":
...
# --- PAGE: DESIGN VAULT (LINK UPDATER) ---
    ...
    if target_id != "Select...":
        ...
        st.write(f"Updating links for: **{target_id}**")
        
        f_link = st.text_input("Paste Front Design Link", value=df.at[idx, 'Image_front'])
        b_link = st.text_input("Paste Back Design Link", value=df.at[idx, 'Image_back'])
        ...
        if st.button("💾 Save Links"):
            ...
            st.success("Links saved to Spreadsheet!")
            st.rerun()
